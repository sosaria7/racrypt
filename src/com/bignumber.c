...
int BnMod(struct RaBigNumber *r, struct RaBigNumber *a, struct RaBigNumber *b)
{
	...
	while (ad >= aa->data) {
		...
		// check the length
		if (rr->length > b->length) {
			ru.low = *rd;
			ru.high = *(rd + 1);
		}
		else if (rr->length == b->length) {
			ru.low = *rd;
			ru.high = 0;
		}
		else {
...
int BnGenRandom(struct RaBigNumber *bn, int bit, struct RaRandom *rnd)
{
	...
	if (bit > 0) {
		bn->data[word - 1] &= ((bn_uint_t)1 << bit) - 1;
	}
	bn->length = word;
	while (bn->length > 1 && bn->data[bn->length - 1] == 0)
		bn->length--;
