...
static void RaAesEncryptData(struct RaAesCtx *ctx, const uint8_t input[16], uint8_t output[16], uint32_t *iv)
{
	...
	if (iv != NULL) {
		memcpy(iv, tmpData, 16);
	}
	memcpy(output, tmpData, 16);
}

static void RaAesEncryptIV(struct RaAesCtx *ctx)
{
	int round;
	uint32_t* tmpData = ctx->iv;
	uint32_t tmpData2[4];
...
static void RaAesDecryptData(struct RaAesCtx *ctx, const uint8_t input[16], uint8_t output[16], uint32_t *iv)
{
	int round;
	uint32_t tmpData[4];
	uint32_t tmpData2[4];
...
static void RaAesEncryptBlock(struct RaAesCtx *ctx, const uint8_t input[16], uint8_t output[16])
{
	uint32_t tmpData[4];
	switch (ctx->opMode) {
	case RA_AES_MODE_ECB: default:
		RaAesEncryptData(ctx, input, output, NULL);
		break;
	case RA_AES_MODE_CBC:
		RaAesEncryptData(ctx, input, output, ctx->iv);
		break;
	case RA_AES_MODE_CFB:
		RaAesEncryptIV(ctx);
		memcpy(tmpData, input, RA_BLOCK_LEN_AES);
		ctx->iv[0] ^= tmpData[0];
		ctx->iv[1] ^= tmpData[1];
		ctx->iv[2] ^= tmpData[2];
		ctx->iv[3] ^= tmpData[3];
		memcpy(output, ctx->iv, RA_BLOCK_LEN_AES);
		break;
	case RA_AES_MODE_OFB:
		RaAesEncryptIV(ctx);
		memcpy(tmpData, input, RA_BLOCK_LEN_AES);
		tmpData[0] ^= ctx->iv[0];
		tmpData[1] ^= ctx->iv[1];
		tmpData[2] ^= ctx->iv[2];
		tmpData[3] ^= ctx->iv[3];
		memcpy(output, tmpData, RA_BLOCK_LEN_AES);
		break;
	}
}

static void RaAesDecryptBlock(struct RaAesCtx *ctx, const uint8_t input[16], uint8_t output[16])
{
	uint32_t tmpData[4];
	switch (ctx->opMode) {
	case RA_AES_MODE_ECB: default:
		RaAesDecryptData(ctx, input, output, NULL);
		break;
	case RA_AES_MODE_CBC:
		RaAesDecryptData(ctx, input, output, ctx->iv);
		break;
	case RA_AES_MODE_CFB:
		RaAesEncryptIV(ctx);
		memcpy(tmpData, input, RA_BLOCK_LEN_AES);
		...
		tmpData[3] ^= ctx->iv[3];
		memcpy(output, tmpData, RA_BLOCK_LEN_AES);
		memcpy(ctx->iv, input, RA_BLOCK_LEN_AES);
		break;
	case RA_AES_MODE_OFB:
		RaAesEncryptIV(ctx);
		memcpy(tmpData, input, RA_BLOCK_LEN_AES);
