...
static int8_t permute8(uint8_t x, uint8_t *a)
{
	int i;
	uint32_t ret = 0;
	uint32_t t;
...
static void printData32(uint32_t *data, int count)
{
	int i;
	printf("\t");
	for (i = 0; i < count; i++)
...
int RaSeedCreate(const uint8_t *key, enum RaBlockCipherMode opMode, struct RaSeedCtx **ctxp)
{
	...
	ctx = (struct RaSeedCtx*)malloc(sizeof(struct RaSeedCtx));
	if (ctx == NULL) {
		return RA_ERR_OUT_OF_MEMORY;
	}
	RaSeedInit(ctx, key, opMode);
...
void RaSeedInit(struct RaSeedCtx *ctx, const uint8_t *key, enum RaBlockCipherMode opMode)
{
	...
	RaBlockCipherInit(&ctx->blockCipher, RaSeedEncryptBlock, RaSeedDecryptBlock, opMode, RA_BLOCK_LEN_SEED, ctx->iv, ctx->buffer);

	a = GET_UINT32_BE(key);
	b = GET_UINT32_BE(key + 4);
	c = GET_UINT32_BE(key + 8);
	d = GET_UINT32_BE(key + 12);
