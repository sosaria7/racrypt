...
static void RaBlowfishEncryptProcess(struct RaBlowfishCtx *ctx, uint32_t *data_l, uint32_t *data_r);
static void RaBlowfishDecryptProcess(struct RaBlowfishCtx *ctx, uint32_t *data_l, uint32_t *data_r);
static void RaBlowfishEncryptBlock(struct RaBlockCipher* blockCipher, const uint8_t* input, uint8_t* output);
static void RaBlowfishDecryptBlock(struct RaBlockCipher* blockCipher, const uint8_t* input, uint8_t* output);
...
int RaBlowfishCreate(const uint8_t *key, int keyLen, enum RaBlockCipherMode opMode, struct RaBlowfishCtx **ctxp)
{
	...
	ctx = (struct RaBlowfishCtx *)malloc(sizeof(struct RaBlowfishCtx));
	if (ctx == NULL) {
		return RA_ERR_OUT_OF_MEMORY;
	}
	RaBlowfishInit(ctx, key, keyLen, opMode);
...
void RaBlowfishInit(struct RaBlowfishCtx *ctx, const uint8_t *key, int keyLen, enum RaBlockCipherMode opMode)
{
	...
	RaBlockCipherInit(&ctx->blockCipher, RaBlowfishEncryptBlock, RaBlowfishDecryptBlock, opMode, RA_BLOCK_LEN_BLOWFISH, ctx->iv, ctx->buffer);

	p = 0;
	for (i = 0; i < 18; i++) {
		k = (key[p] << 24) | (key[(p + 1) % keyLen] << 16) | (key[(p + 2) % keyLen] << 8) | key[(p + 3) % keyLen];
		p = (p + 4) % keyLen;
	...
	data_r = 0;
	for (i = 0; i < 18; i += 2) {
		RaBlowfishEncryptProcess(ctx, &data_l, &data_r);
		ctx->p_array[i] = data_l;
		ctx->p_array[i + 1] = data_r;
	}
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 256; j += 2) {
			RaBlowfishEncryptProcess(ctx, &data_l, &data_r);
			ctx->sbox[i][j] = data_l;
			ctx->sbox[i][j + 1] = data_r;
		}
	}
}


#define BFF(x)	(((ctx->sbox[0][x >> 24] + ctx->sbox[1][x >> 16 & 0xff]) ^ ctx->sbox[2][x >> 8 & 0xff]) + ctx->sbox[3][x & 0xff])
...
static void RaBlowfishEncryptProcess(struct RaBlowfishCtx *ctx, uint32_t *data_ll, uint32_t *data_rr)
{
	uint32_t data_l;
	uint32_t data_r;
	uint32_t data_tmp;
	...
	for (i = 0; i < 16; i += 2) {
		data_l = data_l ^ ctx->p_array[i];
		data_r = BFF(data_l) ^ data_r;

		data_r = data_r ^ ctx->p_array[i + 1];
		data_l = BFF(data_r) ^ data_l;
	}
...
static void RaBlowfishEncryptBlock(struct RaBlockCipher *blockCipher, const uint8_t *input, uint8_t *output)
{
	...
	ctx = CHILD_OF(blockCipher, struct RaBlowfishCtx, blockCipher);

	data_l = GET_UINT32_BE(input);
	input += 4;
	data_r = GET_UINT32_BE(input);

	RaBlowfishEncryptProcess(ctx, &data_l, &data_r);

	PUT_UINT32_BE(output, data_l);
	output += 4;
	PUT_UINT32_BE(output, data_r);
}
...
static void RaBlowfishDecryptBlock(struct RaBlockCipher *blockCipher, const uint8_t *input, uint8_t *output)
{
	...
	RaBlowfishDecryptProcess(ctx, &data_l, &data_r);
